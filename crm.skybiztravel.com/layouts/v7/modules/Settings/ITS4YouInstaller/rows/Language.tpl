            ...
            <span class="extension_container">
                ...
                <span class="pull-left">
                    ...
                    {if $ALL_LANGUAGES[$LANG_KEY] neq ""}
                        ...
                        <button class="oneclickInstallFree isUpdateBtn btn btn-success {if $IS_AUTH}authenticated {else} loginRequired{/if}">{vtranslate('LBL_UPDATE', $QUALIFIED_MODULE)}</button>
                        {/if}
                    {else}
                        <input type="hidden" name="moduleAction" value="Install"/>
